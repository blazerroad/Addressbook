...
export class BaseReduxService<TEntity extends IEntity, TRepository extends IRepository<TEntity>> implements IReduxService<TEntity> {
    ...
    async pagination(actionName: string,take : number , skip : number): Promise<void> {
        const response = await this.repository.pagination(take,skip);
       
        this.dispatch(actionName, response);
    }

    async  query(actionName: string, context: string): Promise<void> {
